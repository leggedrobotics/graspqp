...
def default_selection_func(i: int, num_assets: int) -> int:
    """Default selection function for selecting the asset configuration to spawn.

    Returns the index of the asset configuration to spawn. If the number of assets is greater than the number of asset configurations, the asset configurations are repeated.
    Args:
        i: The index of the asset to spawn
        num_assets: The total number of assets to spawn
    Returns:
        The index of the asset configuration to spawn
    """
    return i % num_assets
    ...
    with Sdf.ChangeBlock():
        for i, prim_path in enumerate(prim_paths):
            ...
            if cfg.randomize:
                ...
                idx = cfg.selection_func(i, len(proto_prim_paths))
...
def _spawn_dataset_objects(
    ...
    if hasattr(asset_cfg, "semantic_tags") and asset_cfg.semantic_tags is not None:
        # note: taken from replicator scripts.graspqp_isaaclab.utils.utils .py
