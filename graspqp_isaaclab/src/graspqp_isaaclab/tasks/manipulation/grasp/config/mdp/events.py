...
def pull_drawer(
    ...
    obj.write_joint_effort_limit_to_sim(
        torch.ones(len(env_ids), 1, device=env_ids.device) * max_force, env_ids=env_ids, joint_ids=[2]
    )
    obj.set_joint_position_target(
        -torch.ones(len(env_ids), 1, device=env_ids.device) * 100, env_ids=env_ids, joint_ids=[2]
    )
