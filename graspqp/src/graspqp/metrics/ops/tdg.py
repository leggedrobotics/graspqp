    ...
    def to_device(self, data_tensor):
        if isinstance(data_tensor, torch.Tensor):
            ...
            return torch.as_tensor(
                np.array(data_tensor), device=self.device, dtype=self.dtype
            )
...
class GraspEnergyBase:
    ...
    def __init__(
        self, miu_coef, obj_gravity_center, obj_obb_length, tensor_args, enable_density
    ):
    ...
    def estimate_density(self, normal):
        cos_theta = (normal.unsqueeze(-2) * normal.unsqueeze(-3)).sum(
            dim=-1
        )  # [b, n, n]
    ...
    def reset(self, gravity_center, obb_length):
        if gravity_center is not None:
            if (
                self.obj_gravity_center is not None
                and gravity_center.shape == self.obj_gravity_center.shape
            ):
        ...
        if obb_length is not None:
            if (
                self.obj_obb_length is not None
                and obb_length.shape == self.obj_obb_length.shape
            ):
...
class TDGEnergy(GraspEnergyBase):
    ...
    def __init__(
        ...
        **args
        ...
        super().__init__(
            miu_coef, obj_gravity_center, obj_obb_length, tensor_args, enable_density
        )
        ...
        self.target_direction_6D = self.tensor_args.to_device(direction_6D).unsqueeze(
            0
        )  # [1, P, 6]

        # self.target_direction_6D = torch.eye(6, device=self.tensor_args.device, dtype=self.tensor_args.dtype).unsqueeze(0)
        self.F_center_direction = self.tensor_args.to_device([1, 0, 0])[
            None, None, None
        ]
    ...
    def GWS(self, G_matrix, normal):
        ...
        proj_on_cn = (direction_F * self.F_center_direction).sum(
            dim=-1, keepdim=True
        )  # [b, P, n, 1]
        perp_to_cn = (
            direction_F - proj_on_cn * self.F_center_direction
        )  # [b, P, n, 3]  or [b, P, n, 4]
        ...
        help3 = perp_norm / (
            perp_norm - 2 * bottom_length * torch.clamp(proj_on_cn, max=0)
        )
        ...
        argmin_3D_on_normalized_cone = (
            ...
            * (
                ...
                + perp_to_cn / torch.clamp(proj_on_cn, min=torch.cos(bottom_angle) / 2)
            )
            + region2 * help2
            + region3 * help3 * help2
        )  # [b, P, n, 3]
        ...
        w = (
            G_matrix.unsqueeze(1) @ argmin_3D_on_normalized_cone.unsqueeze(-1)
        ).squeeze(
            -1
        )  # [b, P, n, 6]
