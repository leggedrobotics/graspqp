...
class LsqSolver:
    ...
    def build_solver_from_mat(self, A, b, step_size=0.15, solver_kwargs={}):
        ...
        self.build_solver(
            A.shape[-1], b.shape[-1], batch_size, device=A.device, step_size=step_size, solver_kwargs=solver_kwargs
        )
    ...
    def build_solver(
        self, num_wrenches: int, wrench_dim: int, batch_size: int = 1, step_size=0.15, device="cuda", solver_kwargs={}
    ):
        ...
        if init is None:
            ...
            if isinstance(init, (int, float)):
                init = (
                    ...
                    .view(1, 1)
                    .expand(n_batches, self._num_wrenches)
                    .clone()
                )
            if init.ndim == 1:
