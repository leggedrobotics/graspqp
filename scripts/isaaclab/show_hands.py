...
AVAILABLE_HANDS = {
    ...
    "robotiq": ROBOTIQ_2F_CFG,
    "schunk": SCHUNK_2F_CFG,
    "panda": PANDA_CFG,
    "robotiq3f": ROBOTIQ_3F_CFG,
    "shadow": SHADOW_HAND_CFG,
}
...
def design_scene() -> tuple[dict, list[list[float]]]:
    ...
    origins = define_origins(num_origins=len(AVAILABLE_HANDS), spacing=0.5)
    scene_entities = {}
    for index, (hand_name, hand_cfg) in enumerate(AVAILABLE_HANDS.items()):
        prim_utils.create_prim(f"/World/Origin{index+1}", "Xform", translation=origins[index])
        # -- Robot
        scene_entities[hand_name] = Articulation(hand_cfg.replace(prim_path=f"/World/Origin{index+1}/Robot"))
